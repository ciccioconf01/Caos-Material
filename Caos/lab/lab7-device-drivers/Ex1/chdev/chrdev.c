...
{ 
    ...
    char message[20];
    int bytes_written = sprintf(message, "%d", sum);
    message[bytes_written] = '\0';  // Assicurati che sia terminata con '\0'
    int i = 0;
...
{ 
    ...
    int MAX_INT = 20;
    char array[MAX_INT];
    int arrayInt[MAX_INT];
    int byte_write;

    if(buff == NULL){
        pr_info("You have to pass the array from input\n");
        return -1;
    }
    //Passing to kernel space
    for(int i = 0; i < len ; i++){
        get_user(array[i],buff+i);
        byte_write++;
    }

    //passing the array from string to int
    string_to_array(array,arrayInt,MAX_INT);
